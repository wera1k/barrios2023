...
if(1){
  ...
  for(i in seq(id)){                    
    if(y3[i] == z[i]){
      ...
      points(id[i], z[i], pch=2, col="lightseagreen")
      segments(id[i], y3[i], id[i], z[i], lty=, col="navy")
    }else{
      points(id[i], y3[i], pch=0, col="navy")
      points(id[i], z[i], pch=2, col="lightseagreen")
 ...
 #x4 <- ideal4[idx]
  ...
  for(i in seq(id)){                    
    if(y4[i] == z[i]){
      ...
      points(id[i], y4[i], pch=0, col="navy")
      points(id[i], z[i], pch=2, col="lightseagreen")
      segments(id[i], y4[i], id[i], z[i], lty=4, col="lightseagreen")
    } 
  }
  abline(v=c(20, 40, 60, 80),lty=3,col=grey(.8))
...
title("Extensión SOC Médicos vs Cirujano", outer=TRUE, line=0, cex.main=2)

par(opar)

savePlots(file="Extensión SOC vs Cirujano", width=4, height=6, EPS=FALSE, PDF=TRUE, PNG=TRUE)
