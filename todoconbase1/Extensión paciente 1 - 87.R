...
# la extensión máxima es 4.18879 pero puse 4.3 para que no quede al borde de la
# gráfica el dato máximo
  ...
  plot(0,0, type="n", xlim=xlim, ylim=ylim, axes=FALSE, frame=TRUE, 
       ann=FALSE, xlab="medico", ylab="extensión")
...
#    savePlots(paste("Extensión por Pacientes pg.",w,sep=""),6,7,FALSE,TRUE,TRUE,FALSE)
    #break()
    next()
  }
  ...
  if(w == 87/12) {
    mtext("Médicos",side=1, line=2, outer=TRUE)
    mtext("Extensión",side=2, line=0, outer=TRUE)
#  savePlots("Extensión por Pacientes pg.8",6,7,FALSE, TRUE,TRUE,FALSE)
  }
